...
private:
  ...
  void getTable()
  {
    Serial.println("Here in getTable");
    Serial.println("switches size: " + String(switches->getSwitches().size()));
...
public:
  WebServerAdapter(IRepoPort *repo, IRepoSwitch *repoSwitches) : server(80), repo(repo), switches(repoSwitches)
  {
    // Serial.println("Repo switches hhheere is : "+String(repoSwitches->getSwitches().size()));
  }

  void begin()
  {
    for (auto &d : repo->getDevices())
    {
      if (d.lightEntity.id >= nextId)
        nextId = d.lightEntity.id + 1;
    }
...
IRepoPort::SmartDevice device{
    Light(nextId++, controllerId, std::string(name.c_str()), std::string(room.c_str())),
    nullptr,
    nullptr 
};
 
  device.lightEntity.state = false;
Serial.println("ok10");
  if (protocol == "GPIO")
  {

    auto *way = new GpioConnectionData();
    way->init(pin);

    device.executeConnection = &gpioInfra;

    device.lightConnectionData = way; 
  ...
  else if (protocol == "ESP-NOW")
  {
    ...
    auto *way = new EspNowWay();
    way->begin(macBytes, pin);
    device.executeConnection = &espNowOutput;
    device.lightConnectionData = way; 
  }
  else
  {
...
Serial.println(ESP.getFreeHeap());
  ...
  bool removed = repo->deleteDeviceById(id);
  ...
  server.on("/api/switch/add", HTTP_POST, [this]()
{
  ...
  if (deviceId == 0 || switchPin < 0)
  {
   ...
   switches->addSwitch(switchPin,deviceId);
   Serial.println("Switch added: Pin " + String(switchPin) + ", Device ID " + String(deviceId));
